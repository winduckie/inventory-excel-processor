...
log_filename = f'logs/advanced_excel_processor_{timestamp}.log'
...
class AdvancedExcelProcessor:
    """
    An advanced Excel processor that can handle multiple tables within sheets.
    """
    ...
    def detect_table_boundaries(self, df: pd.DataFrame) -> List[Tuple[int, int]]:
        """
        Detect table boundaries in a dataframe by looking for patterns.
        
        Args:
            df (pd.DataFrame): Input dataframe
            
        Returns:
            List[Tuple[int, int]]: List of (start_row, end_row) tuples for each table
        """
        boundaries = []
        
        # Method 1: Look for completely empty rows
        empty_rows = df.isna().all(axis=1)
        empty_row_indices = empty_rows[empty_rows].index.tolist()
        
        # Method 2: Look for header patterns (rows with mostly non-empty values)
        header_candidates = []
        for idx, row in df.iterrows():
            non_empty_count = row.notna().sum()
            if non_empty_count >= len(row) * 0.5:  # At least 50% non-empty
                header_candidates.append(idx)
        
        # Method 3: Look for repeated header patterns
        repeated_headers = []
        for i in range(len(df) - 1):
            current_row = df.iloc[i]
            next_row = df.iloc[i + 1]
            if (current_row.notna().sum() > 0 and 
                next_row.notna().sum() > 0 and
                current_row.astype(str).equals(next_row.astype(str))):
                repeated_headers.append(i)
        
        # Combine all separation points
        separation_points = sorted(set(empty_row_indices + repeated_headers))
        
        # Create table boundaries
        start_row = 0
        for sep_point in separation_points:
            if sep_point > start_row:
                boundaries.append((start_row, sep_point - 1))
                start_row = sep_point + 1
        
        # Add the last table
        if start_row < len(df):
            boundaries.append((start_row, len(df) - 1))
        
        # If no boundaries found, treat the entire sheet as one table
        if not boundaries:
            boundaries = [(0, len(df) - 1)]
    ...
    def process_landed_pullout_sheet(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        try:
            ...
            if landed_start is not None and pullout_start is not None:
                ...
                # No headers found, use advanced detection method
                logger.warning("No 'Landed' or 'FOR PULL-OUT' headers found, using advanced detection method")
                boundaries = self.detect_table_boundaries(df)
                logger.info(f"Detected {len(boundaries)} table(s) in LANDED.PULL-OUT sheet")
                
                tables = []
                for i, (start_row, end_row) in enumerate(boundaries):
                    table_df = df.iloc[start_row:end_row + 1].copy()
                    table_df = self._clean_table(table_df)
                    tables.append(table_df)
                    logger.info(f"Table {i+1}: {len(table_df)} rows, {len(table_df.columns)} columns")
                
                # Ensure we have at least two tables
                if len(tables) < 2:
                    # If only one table detected, try to split it differently
                    logger.info("Only one table detected, attempting alternative splitting method")
                    tables = self._alternative_landed_pullout_split(df)
                
                # Store processed data
                landed_table = tables[0] if len(tables) > 0 else pd.DataFrame()
                pullout_table = tables[1] if len(tables) > 1 else pd.DataFrame()
                
                self.sheets_data['LANDED_PULLOUT_table1'] = landed_table
                self.sheets_data['LANDED_PULLOUT_table2'] = pullout_table
    ...
    def _alternative_landed_pullout_split(self, df: pd.DataFrame) -> List[pd.DataFrame]:
        """
        Alternative method to split the LANDED.PULL-OUT sheet when standard detection fails.
        
        Args:
            df (pd.DataFrame): The LANDED.PULL-OUT sheet dataframe
            
        Returns:
            List[pd.DataFrame]: List of separated tables
        """
        tables = []
        
        # Method 1: Look for rows with different column patterns
        column_patterns = []
        for idx, row in df.iterrows():
            pattern = tuple(row.notna())
            column_patterns.append(pattern)
        
        # Find where the pattern changes significantly
        pattern_changes = []
        for i in range(1, len(column_patterns)):
            if column_patterns[i] != column_patterns[i-1]:
                pattern_changes.append(i)
        
        # Split based on pattern changes
        if pattern_changes:
            start = 0
            for change_point in pattern_changes:
                table_df = df.iloc[start:change_point].copy()
                table_df = self._clean_table(table_df)
            ...
            # Add the last table
            if start < len(df):
                table_df = df.iloc[start:].copy()
                table_df = self._clean_table(table_df)
                if not table_df.empty:
                    tables.append(table_df)
        
        # If still no tables, try splitting by data type changes
        if not tables:
            # Look for rows where data types change significantly
            ...
            if len(numeric_columns) > 0 and len(text_columns) > 0:
                ...
                mid_point = len(df) // 2
                table1 = df.iloc[:mid_point].copy()
                table2 = df.iloc[mid_point:].copy()
                
                table1 = self._clean_table(table1)
                table2 = self._clean_table(table2)
    ...
    def combine_product_quantity_data(self) -> pd.DataFrame:
        ...
        Products are categorized based on product_categories.csv.
...
def main():
    """
    Main function to demonstrate the advanced Excel processor usage.
    """
    import os
    ...
    if not combined_df.empty:
        combined_file = os.path.join("processed_data", "ADVANCED_COMBINED_PRODUCT_QUANTITY.csv")
        combined_df.to_csv(combined_file, index=False)
        logger.info(f"Saved advanced combined data to {combined_file}")
        print(f"\n=== ADVANCED COMBINED DATA SUMMARY ===")
